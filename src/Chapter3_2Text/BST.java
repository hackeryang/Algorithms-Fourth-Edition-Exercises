...
public class BST<Key extends Comparable<Key>, Value> {  //基于二叉查找树的符号表
    ...
    public Key select(int k) {  //找到排名为k的键，即树中正好有k个小于它的键
    ...
    public int rank(Key key) {  //返回键的排名，最小键节点排名为0
    ...
    private Node deleteMin(Node x) {
        ...
        //如果存在左子树，则一直递归直到找到没有左子树的最小键的节点，这行当前节点的左子树(x.left)已经指向递归后上一行返回的右链接(x.right)，也就是说，将最小键节点的父节点的左子树链接指向该最小键节点的右子树，这样父节点不再指向该节点，最小键节点会被垃圾回收
    ...
    private Node deleteMax(Node x) {
        ...
        //如果存在右子树，则一直递归直到找到没有右子树的最大键的节点，这行当前节点的右子树(x.right)已经指向递归后上一行返回的左链接(x.left)，也就是说，将最大键节点的父节点的右子树链接指向该最大键节点的左子树，这样父节点不再指向该节点，最大键节点会被垃圾回收
        x.right = deleteMax(x.right);
        x.N = size(x.left) + size(x.right) + 1;   //更新节点计数器
        return x;   //在递归中不断的return当前递归中的节点，一层层往上最终在递归语句那一行return根节点的下一层右子树根节点，这样递归结束后最后在这一行返回根节点
    }

    public void delete(Key key) {  //删除对应键的节点
    ...
    public Iterable<Key> keys() {  //整个二叉查找树的遍历
        return keys(min(), max());
    }
